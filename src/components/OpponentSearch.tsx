...
export default function OpponentSearch(props: {
  ...
  const [opponent, setOpponent] = useState<ApiPlayer | null>(loadOpponent);
  ...
  }, [setOpponents, loadGorChange, id, opponent, win, calcGorResult]);
      ...
      {calcGorResult && (
        <Grid container sx={{ flexGrow: 1 }}>
          <Grid xs={6}>
            ...
            <Typography textAlign={"center"}>GoR change</Typography>
          </Grid>
          <Grid xs={6}>
            <Typography fontSize={30} textAlign={"center"}>
              {(calcGorResult.winProbability * 100).toFixed()}%
            </Typography>
            <Typography textAlign={"center"}>Win probability</Typography>
          </Grid>
        </Grid>
      )}
    </Sheet>
